...
execute as @a[tag=d,scores={aktime=1..}] at @s run titleraw @s actionbar {"rawtext":[{"score":{"name":"@s","objective":"dstime"}},{"text":"後にリスポーン可能"}]}
...
execute as @a[tag=d,scores={aktime=..0},tag=!rsm] at @s run title @s subtitle 救急隊に自動通報しました
